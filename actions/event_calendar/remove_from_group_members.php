...
if ($group instanceof ElggGroup && $event->subtype == 'event_calendar' && $group->canEdit()) {
	...
	elgg_register_success_message(elgg_echo('event_calendar:remove_from_group_members:success'));
} else {
	elgg_register_error_message(elgg_echo('event_calendar:remove_from_group_members:error'));
}
