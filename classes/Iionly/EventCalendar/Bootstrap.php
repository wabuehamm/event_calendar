...
namespace Iionly\EventCalendar;
...
class Bootstrap extends DefaultPluginBootstrap
{
    ...
    {
        elgg_register_event_handler("setting", "plugin", array($this, "event_calendar_invalidate_cache"));

        elgg_register_event_handler('cron', 'fiveminute', array($this, 'event_calendar_handle_reminders_cron'), 400);
        elgg_register_event_handler('entity:url', 'object', array($this, 'event_calendar_url'));
        elgg_register_event_handler(
            'prepare', 
            'notification:create:object:event_calendar', 
            array($this, 'event_calendar_prepare_notification')
        );
        ...
        elgg_entity_enable_capability('object', 'event_calendar', 'searchable');
        ...
        if (!$group_calendar || $group_calendar != 'no') {
            elgg_register_event_handler('register', 'menu:owner_block', array($this, 'event_calendar_owner_block_menu'));
            ...
            elgg()->group_tools->register('event_calendar', [
                ...
                'default_on' => !$event_calendar_group_default || ($event_calendar_group_default == 'yes')
            ]);
        }
        ...
        elgg_register_widget_type([
            'id' => 'event_calendar',
            'name' => elgg_echo("event_calendar:widget_title"),
            'description' => elgg_echo('event_calendar:widget:description'),
        ]);
        ...
        if (elgg_is_active_plugin('widget_manager')) {
            ...
            elgg_register_widget_type([
                'id' => 'index_event_calendar', 
                'title' => elgg_echo("event_calendar:widget_title"), 
                'description' => elgg_echo('event_calendar:widget:description'), 
                'context' => ["index"]
            ]);
            if (!$group_calendar || $group_calendar != 'no') {
                elgg_register_widget_type([
                    'id' => 'groups_event_calendar', 
                    'title' => elgg_echo("event_calendar:widget_title"), 
                    'description' => elgg_echo('event_calendar:widget:description'), 
                    'context' => ["groups"]
                ]);
            ...
            elgg_register_event_handler("entity:url", "object", array($this, "event_calendar_widget_urls"));
            // handle the availability of the Event Calendar group widget
            elgg_register_event_handler(
                "group_tool_widgets", 
                "widget_manager", 
                array($this, "event_calendar_tool_widgets_handler")
            );
        }

        // if autogroup is set, listen and respond to join/leave events
        if (elgg_get_plugin_setting('autogroup', 'event_calendar') == 'yes') {
            elgg_register_event_handler('join', 'group', array($this, 'event_calendar_handle_join'));
            elgg_register_event_handler('leave', 'group', array($this, 'event_calendar_handle_leave'));
        }

        // entity menu
        elgg_register_event_handler('register', 'menu:entity', array($this, 'event_calendar_entity_menu_setup'));
        elgg_register_event_handler('prepare', 'menu:entity', array($this, 'event_calendar_entity_menu_prepare'));
        ...
        elgg_register_event_handler('likes:is_likable', 'object:event_calendar', 'Elgg\Values::getTrue');
        ...
        elgg_register_event_handler('upgrade', 'system', array($this, 'event_calendar_check_pending_upgrades'));

        if (elgg_get_plugin_setting('ical_import_export', 'event_calendar') == "yes") {
            elgg_register_event_handler('register', 'menu:title', array($this, 'ec_ical_titlemenu'));
        }
    }

    public function event_calendar_invalidate_cache($hook, $type, $return, $params){
        if(isset($params["plugin"]) && ($params["plugin"]->getID() == "event_calendar")){
            elgg_clear_caches();
        }
    }

    public function event_calendar_handle_reminders_cron() {
     ...
     */
    public function event_calendar_url($hook, $type, $url, $params) {
     ...
     */
    public function event_calendar_prepare_notification($hook, $type, $notification, $params) {
     ...
     */
    public function event_calendar_owner_block_menu($hook, $type, $return, $params) {
        require_once(elgg_get_plugins_path() . 'event_calendar/models/model.php');
        ...
        } else if ($params['entity'] instanceof ElggUser) {
    ...
    public function event_calendar_widget_urls($hook_name, $entity_type, $return_value, $params){
        $result = $return_value;
    ...
    public function event_calendar_tool_widgets_handler($hook, $type, $return_value, $params) {
    ...
    public function event_calendar_handle_join($event, $object_type, $object) {
        require_once(elgg_get_plugins_path() . 'event_calendar/models/model.php');
        $group = $object['group'];
        $user = $object['user'];
    ...
    public function event_calendar_handle_leave($event, $object_type, $object) {
        require_once(elgg_get_plugins_path() . 'event_calendar/models/model.php');
        $group = $object['group'];
        $user = $object['user'];
        $user_guid = $user->getGUID();
     ...
     */
    public function event_calendar_entity_menu_setup($hook, $type, $return, $params) {
        ...
        if ($user_guid) {
            ...
            if ($calendar_status == 'open') {
                ...
                if (!event_calendar_has_personal_event($entity->guid, $user_guid) && ! $entity->hasRelationship($user_guid, 'event_calendar_request')) {
                    $options = [
                        'name' => 'personal_calendar',
    ...
    public function event_calendar_entity_menu_prepare($hook, $type, $return, $params) {
    ...
    public function event_calendar_check_pending_upgrades() {
        require_once(elgg_get_plugins_path() . 'event_calendar/models/model.php');
    ...
    public function ec_ical_titlemenu($hook, $type, $return, $params) {
